...
export const STAGES: readonly Stage[] = [
  {
    ...
    narration: 'Alice signs in to Keycloak and consents to the agent acting for her. She gets an ordinary OIDC access token that says who she is and what she consented to — and nothing at all about the software acting for her.',
  ...
  },
  {
    ...
    narration: 'SPIRE runs as an intermediate CA under the lab’s offline root (EJBCA), so every workload certificate chains back to real PKI. The agent asks for its own identity and receives two forms of the same SPIFFE ID: an X.509-SVID for mTLS, and a JWT-SVID it uses instead of a client secret. Nothing is typed or configured — attestation issues both.',
  ...
  },
  {
    ...
    proves: 'that the token cannot travel. It is bound to the workload that was issued it — a stolen copy is useless from any other peer.',
  },
  {
...
export function custodyHops(run: DemoRun): readonly CustodyHop[] {
  return [
    ...
    },
    {
      ...
      detail: 'issued offline by EJBCA (M3)',
...
const REJECTION_COPY: Readonly<Record<string, Omit<RejectionCard, 'id' | 'verdict' | 'failed'>>> = {
  'no-client-cert': {
    ...
    lesson: 'It never reaches application code — there is nothing to bypass.',
  },
  'wrong-audience': {
    story: 'A valid Keycloak token minted for another service is replayed here.',
    mechanism: 'aud check (anti-passthrough)',
    lesson: 'A token is for one resource. Passing tokens through is not delegation.',
  },
  'svid-as-bearer': {
    story: 'The workload’s JWT-SVID is sent straight to the MCP server as a bearer token.',
    mechanism: 'issuer / JWKS',
    lesson: 'Identity is not authorization. An SVID says who you are, not what you may do.',
  },
  'unlisted-workload': {
    story: 'A correctly exchanged token arrives from a workload nobody registered.',
    mechanism: 'workload allowlist',
    lesson: 'Valid identity still needs an explicit invitation.',
  },
  'token-replay': {
    story: 'Another allowlisted workload steals the token and replays it over its own mTLS.',
    mechanism: 'act.sub == mTLS peer',
    lesson: 'The token is bound to the workload it was issued to. This is the whole point of act.',
  },
  'insufficient-scope': {
    story: 'The model decides to read the audit log — outside what Alice consented to.',
    mechanism: 'token scopes (user ∩ agent)',
    lesson: 'The model was willing, the token said no.',
  },
};
...
export function rejectionCards(run: DemoRun): readonly RejectionCard[] {
  const cards: RejectionCard[] = run.rejections.map((r) => ({
    ...
    ...(REJECTION_COPY[r.id] ?? { story: r.title, mechanism: r.source, lesson: '' }),
  }));
