#!/usr/bin/env bash
# Idempotent Keycloak "ai-agents" realm for M4+ (kcadm.sh inside the container).
#   realm ai-agents
#   user  alice / alice-password
#   client-scope mcp-audience: audience mapper -> https://mcp.ai-agent.id.eviden.internal:8443 (D-005)
...
if K get realms/ai-agents --fields realm >/dev/null 2>&1; then say "realm ai-agents exists"; else
  say "creating realm ai-agents"; K create realms -s realm=ai-agents -s enabled=true
fi

if has alice get users -r ai-agents -q username=alice --fields username; then
  say "user alice exists"
else
  say "creating user alice"
  K create users -r ai-agents -s username=alice -s enabled=true
fi
...
K update "users/$ALICE_ID" -r ai-agents -s email=alice@ai-agent.id.eviden.internal -s firstName=Alice \
  -s lastName=Demo -s emailVerified=true -s 'requiredActions=[]'
K set-password -r ai-agents --username alice --new-password alice-password
...
if [ -n "$SCOPE_ID" ]; then say "client-scope mcp-audience exists ($SCOPE_ID)"; else
  ...
  SCOPE_ID=$(K create client-scopes -r ai-agents -s name=mcp-audience -s protocol=openid-connect -i)
  K create "client-scopes/$SCOPE_ID/protocol-mappers/models" -r ai-agents \
    -s name=mcp-aud -s protocol=openid-connect -s protocolMapper=oidc-audience-mapper \
    -s 'config."included.custom.audience"=https://mcp.ai-agent.id.eviden.internal:8443' \
    -s 'config."access.token.claim"=true'
fi

client_id() { K get clients -r ai-agents -q clientId="$1" --fields id 2>/dev/null | tr -d ' \n' | sed 's/.*"id":"\([^"]*\)".*/\1/'; }

if has test-caller get clients -r ai-agents -q clientId=test-caller --fields clientId; then
  say "client test-caller exists"
else
  say "creating client test-caller"
  K create clients -r ai-agents -s clientId=test-caller -s publicClient=true \
    -s directAccessGrantsEnabled=true -s enabled=true
fi
CID=$(client_id test-caller)
K update "clients/$CID/default-client-scopes/$SCOPE_ID" -r ai-agents
say "test-caller default scope mcp-audience attached"

if has wrong-aud-client get clients -r ai-agents -q clientId=wrong-aud-client --fields clientId; then
  say "client wrong-aud-client exists"
else
  say "creating client wrong-aud-client (no mcp audience — anti-passthrough source)"
  K create clients -r ai-agents -s clientId=wrong-aud-client -s publicClient=false \
    -s serviceAccountsEnabled=true -s secret=wrong-aud-secret -s enabled=true
fi

say "realm ai-agents ready"
