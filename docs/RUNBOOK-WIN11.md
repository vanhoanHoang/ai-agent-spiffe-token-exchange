...
| Hop 1 works, hop 2 fails: console says `certificate service refused ... EOF reached while reading`; cert-service log shows `HTTP/1.1 header parser received no bytes` in `EjbcaEnrollment.issue` | The RA keystore in `infra/pki/ra/` was not issued by the EJBCA running now (folder copied from another machine, or the ejbca-data volume recreated after `setup-employee-profile.sh`). Both PKI scripts used to skip on file existence alone. | `bash infra/pki/setup-employee-profile.sh` (now re-creates when the running EJBCA disagrees), then `docker compose -f infra/docker-compose.yml restart cert-service agent-pki`. `demo/checklist.sh` probes this connection ("EJBCA accepts the RA keystore"). |
