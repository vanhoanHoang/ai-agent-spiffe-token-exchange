...
## D-040 — PKI contract files are checked for consistency with the running EJBCA, not for existence; the checklist exercises the RA plane

Date: 2026-09-11 · Milestone: demo hardening (post-M15) · Author: claude-code, from a live failure on the company laptop

Context: on the company laptop the first two-hop demo failed at hop 2 only. Console: `certificate service refused: ... EOF reached while reading`. cert-service log: `java.io.IOException: HTTP/1.1 header parser received no bytes` at `EjbcaEnrollment.issue`. Everything else was green: the realm, SPIRE, hop 1, the mTLS call into cert-service, the audit chain `agent-pki <- agent-client`. Per CLAUDE.md §8, two hypotheses: (1) the RA keystore in `infra/pki/ra/` was not issued by the ManagementCA of the EJBCA running on that machine; (2) EJBCA not yet up. The same screenshot showed EJBCA healthy minutes before the call, so (2) was out. The symptom itself is (1): with TLS 1.3 the server's certificate alert arrives after the handshake, so the JDK client sees a completed handshake, writes the request, and reads zero bytes. Reproduced here by planting a P12 from another CA: identical closed connection (curl exit 56, no HTTP status).

Findings:

1. **Both PKI scripts exited early on file existence alone.** `setup-ejbca.sh` and `setup-employee-profile.sh` treated "the contract files are here" as "nothing to do", before even starting EJBCA. Files copied from another machine, or an `ejbca-data` volume recreated after the scripts ran, therefore survived every one-time setup and every reset. The preflight's "hierarchy vs volume" check only caught the case where the volume was absent. Fix: both scripts start EJBCA first and compare the file with the CA actually running — `setup-employee-profile.sh` compares `ra/ejbca-tls-ca.pem` to `ca getcacert ManagementCA` by SHA-256 fingerprint and verifies the P12's certificate against it, re-creating on mismatch; `setup-ejbca.sh` compares `ejbca-root.pem` to the live EvidenRoot and stops with the runbook's resolution on mismatch, because SPIRE and Keycloak trust that file and re-issuing behind their back would be worse than failing.
2. **`--force` on the same EJBCA never produced a new keystore.** After the first batch the end entity sits at status GENERATED (40; verified live with `ra findendentity`) and `batch` generates nothing, so the script copied a stale or absent file. Fix: when the end entity exists, `ra setendentitystatus --username ra-cert-service -S 10` (NEW, per the live `--help`) before `batch`. Exercised here: the re-creation path produced a keystore EJBCA accepts (REST status 200) and M12 passed end to end with it.
3. **"Restart not needed" was wrong once cert-service had tried.** The RA `HttpClient` is cached after its first build; agent-pki caches its MCP session to cert-service. After re-creating the credential, both must restart (`restart cert-service agent-pki`); restarting only cert-service yields `MCP session with server terminated` at hop 2. Script output, checklist hint and runbook say so now.
4. **The checklist now exercises the connection the way cert-service does** (`curl --cert-type P12 ... --resolve ejbca:8444:127.0.0.1` against the REST status endpoint): 200 is OK, a closed connection names the cause and the fix. agent-pki and cert-service joined the health list; ejbca did not (the pinned image has no HEALTHCHECK, verified with `docker image inspect`), the probe covers it. Verified: FAIL with the planted keystore, PASS after re-creation. This is the "verify like the demo does, not like a file check" lesson again, on the PKI plane.
5. **cert-service names the cause.** The bare `IOException` is rethrown as an `IllegalStateException` that says the keystore is not accepted by the running EJBCA and points at the checklist. Rethrown, never permitted.
6. **A second, cosmetic laptop defect on the same run:** every `up` printed `! agent-web Warning pull access denied for spiffe-mcp-lab-agent-client ... may require 'docker login'` because that service declares both `image:` and `build:` and compose probes a registry first. Harmless (the container was created from the local image 0.2 s later) but it reads as a broken launch. `pull_policy: never` on agent-web; runbook row for older checkouts. The `http2: server: error reading preface from client //./pipe/dockerDesktopLinuxEngine` line next to it is Docker Desktop pipe noise on Windows.

Consequences: on the laptop the fix is `git pull`, then `bash infra/pki/setup-employee-profile.sh` (it now detects the mismatch itself) and `docker compose -f infra/docker-compose.yml restart cert-service agent-pki`, then `demo/checklist.sh` must print `EJBCA accepts the RA keystore ... OK`. If `setup-ejbca.sh` reports the root mismatch instead, the files and the volume are from different machines and the runbook's delete-and-redo row applies. The `infra/pki/README.md` contract is unchanged; what changed is that "present" now means "present and issued by the EJBCA in the volume next to it".

Addendum (same day, from the laptop): with the consistency checks in place the script reported the laptop's credential as matching the live ManagementCA — anchor fingerprint equal, leaf chain-valid — and EJBCA still closed the connection. `setup-employee-profile.sh --force` (the status-reset path) issued a fresh keystore and hop 2 issued a real certificate. Root cause of the refusal of a chain-valid P12 is not established (no EJBCA-side log was captured). Consequence: chain-valid is not the same as accepted, so the script's early exit now requires EJBCA's own verdict — the REST status call over mTLS with the P12, the exact call cert-service makes — and re-creates on anything but 200. The offline checks remain as the explanation printed before re-creating.

## D-041 — Identity cards: every certificate the console shows is one card under the step that produces it; the JWT-SVID and the server's certificate join the page

Date: 2026-09-11 · Milestone: demo track, post-P6 · Author: claude-code (design agreed with the human from an ASCII mock; branch `p7-identity-cards`)

Context: the console showed the agent's own X.509-SVID as a bespoke chain panel and the issued employee certificate as a second, differently shaped panel. Two things were missing that the story turns on — the JWT-SVID (the workload's second proof, the one Keycloak sees) and the certificate the MCP server presents in the handshake (the other half of "mutual") — and the issued certificate carried no chain. The human asked for "simple and elegant, like the antoine panel, for both", and for less prose on the page.

Decisions:

1. **One card, three placements.** `dc-cert-card` (kicker, one lead sentence, the openssl `x509 -text` view, then `chain (n)` and `PEM` behind disclosures) renders every certificate. It sits under the step that produces it: the SVID step shows the X.509-SVID card and, beside it, the JWT-SVID as the existing token card (decoded claims, signature redacted — the console's no-token rule); the mTLS step shows the server's certificate; the end of the chain shows the issued certificate, now with its chain. No tabs, no second renderer. The intermediate's URI name constraint — the demo's technical control — stays visible as a one-line note on its chain row, so nothing the old expander showed was lost.
2. **Endpoints are additive and metadata-only.** `/api/svid` keeps its shape (check-p6 still asserts it, and check-cert-cards re-asserts "no key, no PEM"). New: `/api/jwt-svid` mints a JWT-SVID for the audience the real client authentication uses — the AS issuer identifier as the sole value (CLAUDE.md §3) — decodes it with Nimbus, and returns header + claims + `signature: "REDACTED"`; the token never leaves the method. `/api/peer?target=` opens one TLS session with the SAME accept-only-this-SPIFFE-ID `SSLContext` the agent's calls use, reads the session's peer chain, and closes; a wrong peer fails the handshake rather than being displayed, and an unknown target is 400. `/api/issued` gains `chain` (leaf, issuers upward, self-signed last = trust anchor) next to the unchanged fields. One JSON shape for all three (`CertJson`).
3. **The chain travels from the issuer.** cert-service asks EJBCA for `include_chain` (probed live on 9.3.7: `certificate_chain` is an array of base64 DER, issuers upward) and adds `chain_pem` to the tool result; agent-pki relays text unchanged; agent-client's holder now captures every PEM block in the result (deduplicated, leaf first) and hands the model one placeholder per block, as before. For the employee profile the chain is leaf + Eviden Root CA, and that is what is drawn — no invented intermediate.
4. **Less to read on stage.** The run header ("Two identities, one call" plus the three URI chips) is gone from the console page — the top bar already names the trust domain. The live chat header names the human only; the scope sentence ("no task permissions yet…", "may act on…", "no mcp:audit") is gone: the consent wall says it when it matters (D-038). The login page keeps the title, the three beats and the button; the lede and the hint under the button are gone. The hop trace no longer appends `(aud=…)` to the SVID line — the audience is now on the JWT-SVID card where it can be read properly.
5. **No breaking change, verified.** Exit check `scripts/check-cert-cards.sh`: /api/svid unchanged, /api/jwt-svid decoded and redacted with aud = issuer and no JWT-shaped string, /api/peer names mcp-server with leaf + trust anchor and refuses unknown targets, /api/issued 204 or with chain, then the whole console exit check (lint, 65 tests including a negative per new card, build, offline). check-m12 green with the chain in the tool result. Not exercised live in this session: a console-driven issuance showing the issued card's chain (needs the LLM in the loop); it is covered by the page unit test and the holder's multi-block capture.

Consequences: the console's data rule gains a second same-origin client (`LiveIdentityClient`) next to `LiveClient`, so P6's surface and tests stay untouched. The agent-web image must be rebuilt for the trimmed event text and the console. `EjbcaEnrollment.Issued` gains `chainPem`; the seam branch (`i0-idpki-seam`, D-040's companion work) must rebase over it — the IDPKI implementation will have to return a chain too, which is right: an issuer that cannot say who signed it is not one we should draw.
