...
#  - Zero LLM credentials exist anywhere (local Ollama, compose profile
#    "demo") — asserted by grep, not by promise.
...
# No API-key-shaped variable anywhere in agent-client source/config or compose.
keys=$(grep -rniE '(api[_-]?key|OPENAI|ANTHROPIC_|GROQ|OPENROUTER)' \
  agent-client/src agent-client/pom.xml infra/docker-compose.yml 2>/dev/null || true)
[ -z "$keys" ] || fail "LLM-credential-shaped reference found: $keys"
echo "OK: zero LLM credentials in source, config, and compose"
