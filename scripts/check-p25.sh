#!/usr/bin/env bash
# M10 P2.5 exit criterion (DEMO-PLAN P2.5):
#  - Scripted authorization-code login (curl through the Keycloak login form,
#    cookie jar, PKCE, CONSENT accepted — the delegation moment), then a chat
#    POST, produces an MCP call whose server log shows sub=alice and
#    act.sub = the agent's SPIFFE ID.
#  - No access token appears in ANY browser-visible response (token hygiene:
#    alice's token lives server-side; the browser holds a session cookie).
...
# equivalent hosts-file line (documented for P5).
set -euo pipefail
cd "$(dirname "$0")/.."
...
# 1) Unauthenticated front page: a login link, and nothing token-shaped.
curl -s -c "$JAR" "$WEB/" | save front.html | grep -q '/oauth2/authorization/keycloak' \
  || fail "front page lacks login link"
echo "OK: front page offers login"

# 2) Start the authorization-code flow -> Keycloak login form.
...
# 4) Consent screen — the delegation moment. Accept it (generic form replay:
#    every hidden input + the accept button).
...
EOF
  ...
  curl -s -b "$JAR" -c "$JAR" -L "${RESOLVE[@]}" "${CARGS[@]}" -d accept=Yes "$CACTION" | save after-consent.html >/dev/null
...
# 5) Authenticated page: chat form present, CSRF token available.
curl -s -b "$JAR" -c "$JAR" "$WEB/" | save home.html | grep -q 'action="/chat"' \
  || fail "authenticated page lacks chat form: $(head -c 300 "$WORK/home.html")"
CSRF=$(grep -o 'name="_csrf"[^>]*value="[^"]*"' "$WORK/home.html" | head -1 | sed 's/.*value="\([^"]*\)".*/\1/')
[ -n "$CSRF" ] || fail "no CSRF token on chat form"
echo "OK: logged in via browser flow; chat form rendered"

# 6) Chat: natural language -> MCP call, asserted in the server log.
T0=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
curl -s -b "$JAR" -c "$JAR" -L --max-time 300 \
  --data-urlencode "message=Use the whoami tool, then state which workload and which human you are acting for." \
  --data-urlencode "_csrf=$CSRF" "$WEB/chat" | save chat.html >/dev/null
...
if grep -hE "$JWT_RE" "$WORK"/front.html "$WORK"/home.html "$WORK"/chat.html "$WORK"/after-consent.html >/dev/null 2>&1; then
