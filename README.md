...
> Moving to a Windows 11 demo laptop? Follow **[docs/RUNBOOK-WIN11.md](docs/RUNBOOK-WIN11.md)** —
> the same steps as below, in copy-paste order, plus every failure seen on Windows and its fix line.
> `demo/export-images.sh` / `demo/import-images.sh` carry the built images on a USB stick so the
> venue's wifi is not on the critical path.
...
keystore, docker volumes, the Ollama model, `infra/.env`) is machine-local by
design and is regenerated by the scripts below — a new machine becomes a new
lab instance with its own root CA, which is intended.
...
### 3. Choose the LLM (optional — the default needs nothing)

The model is a deployment detail, selected by environment (D-039): the
identity architecture is identical either way (the model picks *what* tool to
call; SVID → token exchange → mTLS decides *as whom*).

| `LLM_PROVIDER` | What runs | Needs |
|---|---|---|
| `ollama` (default) | local `qwen3.5:4b` in the `ollama` container | nothing; ~2–3 GB one-time pull, CPU inference (slow on a laptop) |
| `openai` | any OpenAI-wire hosted endpoint — Groq by default | internet + a key; no container, no pull |

For a demo laptop without a GPU, use Groq's free tier (key from
console.groq.com, no card): create `infra/.env` (gitignored — the only place
a key may live) with
...
`LLM_BASE_URL` and `LLM_MODEL` have defaults in `infra/llm-env.sh`; override
them there or in `.env` for another OpenAI-compatible endpoint. The preflight
(`demo/up.sh`) verifies the key and the model catalog before anything starts.
The default configuration holds zero credentials — `scripts/check-p2.sh` and
`scripts/check-llm-provider.sh` assert that.

### 4. Build and launch (one command block, run from the repo root)
...
# everything else: preflight (free ports, artifacts present, LLM provider),
# image builds (incl. console), services, SPIRE registrations, Keycloak realm
# (incl. two-hop delegation), demo + pki profiles, model pull (local provider
# only), full verification
bash demo/up.sh
```
...
First run is the slowest: all image builds plus, on the local provider, the
~2–3 GB Ollama model download (a hosted provider skips the download and the
container entirely). Subsequent `reset.sh --full` runs take ~3 minutes.

### 5. Hosts file (once, for browser demos)
...
```bash
bash demo/checklist.sh --full     # clock skew FIRST, health, model (local or hosted), full acceptance
```
