...
case "$MODE" in
  --cold)
    ...
    # The contract files and the volumes are one unit (D-040): with the volumes
    # gone these files describe a CA that no longer exists, and setup-ejbca.sh
    # now refuses to run against a mismatch. Remove them so the rebuild is one
    # command. Keys are regenerated by the setup scripts; nothing here is secret
    # state worth keeping once its CA is gone. SPIRE's bootstrap CA stays: it
    # lives in files, not volumes.
    rm -f infra/pki/*.pem infra/pki/*.srl
    rm -rf infra/pki/ra
    echo "Volumes and their contract files gone. Re-running full setup (EJBCA hierarchy takes a while)..."
